...
Our central finding is that \textbf{multilingual SFT is necessary and English-only SFT does not transfer cross-lingually} in our setup. Across a three-stage evaluation (base $\rightarrow$ English-only SFT $\rightarrow$ multilingual SFT), English-only fine-tuning produces near-zero improvement on non-English sentiment tasks, while multilingual SFT yields substantial gains: Hebrew 53\%$\rightarrow$84.5\%, Arabic 45\%$\rightarrow$60.5\%, Farsi 60.5\%$\rightarrow$78.5\% (logprob classification on balanced evaluation sets).
...
This paper addresses a specific empirical question: \emph{Can efficient multilingual pretraining on a linguistically-motivated language cluster produce useful cross-lingual transfer, and what are the requirements for downstream task transfer?}

We investigate this through three experiments:
\begin{enumerate}
    \item \textbf{Experiment A} (Hebrew Downstream): How well does multilingual SFT enable sentiment classification across all four languages?
    \item \textbf{Experiment B} (Cross-lingual Transfer): Does English-only SFT transfer to non-English languages, or is multilingual SFT required?
    \item \textbf{Experiment C} (Tokenizer Ablation): How much efficiency does a custom balanced tokenizer provide over English-centric alternatives?
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Evidence that \textbf{English-only SFT does not transfer} cross-lingually at this scale---multilingual SFT is necessary for non-English task performance.
...
We train a 32,000-token SentencePiece BPE tokenizer on a balanced sample from all four languages (approximately 25\% each). This ensures no single language dominates the vocabulary.
...
Hebrew is intentionally overrepresented to ensure strong anchor-language representations. Data preprocessing includes MinHash deduplication, perplexity-based quality filtering, and language identification verification.
...
\begin{itemize}
    \item \textbf{Logprob}: Compare log-probabilities of ``positive'' vs ``negative'' completion tokens given the input text and a classification prompt. This avoids generation parsing issues. $N$=200 per language.
    \item \textbf{Generative}: The model generates a free-form classification response. Outputs are parsed for sentiment labels. $N$=50 per language.
\end{itemize}
...
\textbf{Cross-lingual retrieval}: Given a sentence in language A, retrieve its translation from 10 candidates in language B using cosine similarity of mean-pooled hidden states. Chance = 10\%.

\subsection{Experiment A: Multilingual Downstream Evaluation}
\label{sec:exp_a}

We evaluate sentiment classification at three model stages: base (pretrained only), English-only SFT, and multilingual SFT.
...
This contrasts with findings in larger models where English fine-tuning can transfer cross-lingually \citep{conneau2020unsupervised}. At 3B scale with 4.48B pretraining tokens, \textbf{explicit multilingual SFT data is required} for each target language.
...
We compare our custom 32K tokenizer against two baselines of identical vocabulary size: Llama-2 (English-centric) and HebrewGPT-1B (Hebrew-centric). All three use 32K BPE vocabularies.
...
To illustrate the practical impact, consider the Arabic sentence ``عاصمة فرنسا هي باريس.'' (The capital of France is Paris):

\begin{itemize}
    \item \textbf{Ours}: 5 tokens (▁عاصمة | ▁فرنسا | ▁هي | ▁باريس | .)
...
\textbf{Why does this matter for practitioners?} Researchers building models for low-resource languages cannot rely on cheap English-only SFT and hope for cross-lingual transfer. They must invest in multilingual SFT data for each target language---but the good news is that relatively small amounts suffice (our SFT uses only 36,980 total samples).

\subsection{Representation vs.\ Task Transfer}

An intriguing asymmetry: the base model develops strong cross-lingual \emph{representations} (90\% EN$\leftrightarrow$HE retrieval) but cannot leverage them for \emph{tasks} without explicit SFT. Multilingual pretraining builds a shared semantic space, but task-specific fine-tuning is the key that unlocks it.
...
Persian consistently shows weak cross-lingual connectivity despite sharing Arabic script. In our retrieval experiment, Persian achieves only 10--30\% accuracy with other languages, compared to 40--90\% for the Hebrew-Arabic-English triad. This suggests that \textbf{in our setup, linguistic family membership predicts transfer better than script similarity}, though we cannot fully disentangle this from Persian's 20\% pretraining share vs Hebrew's 40\%.

\subsection{Data Quality Lessons}
\label{sec:data_lessons}

During development, we discovered critical data bugs that invalidated earlier results:
...
Based on our experience, we provide recommendations for researchers building similar multilingual models.

\subsection{Language Cluster Selection}

\begin{itemize}
    \item Prioritize \textbf{linguistic family membership} over geographic proximity or script similarity. Our results suggest that Semitic family languages (Hebrew, Arabic) show stronger mutual transfer than script-mates (Arabic, Persian).
    \item Include one high-resource language (English) for evaluation infrastructure and instruction-following bootstrapping.
    \item Include a typologically distant language as a control condition.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{bfloat16} is sufficient---we observed no degradation compared to float32 in preliminary experiments.
...
\begin{itemize}
    \item \textbf{Include all target languages} in SFT data. English-only SFT does not transfer at 3B scale.
...
\midrule
Language selection & Linguistic family $>$ script similarity \\
...
\begin{itemize}
    ...
    \item \textbf{No external baselines}: We do not compare against mBERT, XLM-R, or NLLB on identical tasks. Such comparisons would contextualize our results.
...
\begin{enumerate}
    \item \textbf{Multilingual SFT is required}: English-only fine-tuning does not transfer to non-English tasks at 3B scale. Explicit multilingual SFT data unlocks the cross-lingual representations built during pretraining.
    ...
    \item \textbf{Representations precede tasks}: The model builds strong cross-lingual embeddings (90\% EN$\leftrightarrow$HE retrieval) from pretraining alone, but task-specific SFT is needed to leverage them.
\end{enumerate}

We release all code, data, and model weights to support reproducible research and provide a practitioner's guide for researchers targeting other under-resourced language families.
