...
\author{%
  ...
  \small\textsuperscript{1}Independent Researcher \quad \texttt{ronnen@gmail.com}\\[2pt]
...
\begin{enumerate}
    \item A \textbf{reproducible, efficient recipe} for training a 3B multilingual model from scratch on a single H100 node in under 23 hours.
...
Training uses a single AWS p5en.48xlarge node (8$\times$NVIDIA H100 80GB) with FSDP (Fully Sharded Data Parallelism). Configuration:
...
Total pretraining time: approximately 22.4 hours on a single H100 node.
...
\begin{enumerate}
    ...
    \item \textbf{Massive efficiency for target languages}: Hebrew uses 66\% fewer tokens, Arabic 49\% fewer, Persian 69\% fewer than Llama-2. This directly translates to longer effective context windows and reduced training time.
...
\midrule
Proxy experiments (32 runs) & 1$\times$L40S & $\sim$16h \\
Pretraining (4.48B tokens) & 1$\times$ p5en.48xlarge & $\sim$22.4h \\
SFT (36,980 samples) & 1$\times$A10G & $\sim$1h \\
Evaluation (all experiments) & 1$\times$A10G + CPU & $\sim$3h \\
\bottomrule
\end{tabular}
\caption{Compute requirements. Pretraining dominates; SFT and evaluation are negligible.}
\label{tab:compute}
\end{table}
...
\begin{itemize}
    ...
    \item Use \textbf{spot instances with checkpointing} every 30 minutes to handle preemptions.
...
Compute & Single H100 node, $\sim$23h pretraining \\
\bottomrule
\end{tabular}
...
We present SemiticGPT, a 3.14B parameter multilingual model covering Hebrew, Arabic, Persian, and English, trained from scratch on a single H100 node in under 23 hours. Our three experiments establish that:
