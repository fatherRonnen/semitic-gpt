...
\textbf{Hebrew:} ``הבינה המלאכותית משנה את העולם'' (Artificial intelligence is changing the world):
\begin{itemize}
    \item \textbf{Ours}: 4 tokens (▁הבינה | ▁המלאכותית | ▁משנה | ▁את\_העולם)
    \item \textbf{Llama-2}: 18 tokens (character-level fragmentation)
    \item \textbf{HebrewGPT}: 5 tokens (comparable---Hebrew-optimized)
\end{itemize}

\textbf{Arabic:} ``عاصمة فرنسا هي باريس.'' (The capital of France is Paris):
