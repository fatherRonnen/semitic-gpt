...
\begin{enumerate}
    \item A \textbf{reproducible, efficient recipe} for training a 3B multilingual model from scratch on a single 8$\times$H100 node in under 23 hours.
...
Training uses an AWS p5en.48xlarge instance (8$\times$NVIDIA H100 80GB) with FSDP (Fully Sharded Data Parallelism). Configuration:
...
Total pretraining time: approximately 22.4 hours on 8$\times$H100.
...
Pretraining (4.48B tokens) & 8$\times$H100 & $\sim$22.4h \\
...
Compute & 8$\times$H100, $\sim$23h pretraining \\
\bottomrule
\end{tabular}
...
We present SemiticGPT, a 3.14B parameter multilingual model covering Hebrew, Arabic, Persian, and English, trained from scratch on a single 8$\times$H100 node in under 23 hours. Our three experiments establish that:
