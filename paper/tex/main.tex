...
We provide a step-by-step practitioner's guide covering tokenizer design, data curation, pretraining, supervised fine-tuning, and evaluation, intended for researchers building similar models for other under-resourced language clusters. We release the model, tokenizer, and training code.\footnote{Code and artifacts: \url{https://github.com/fatherRonnen/semitic-gpt}}
\end{abstract}
...
We present SemiticGPT, a 3.04-billion parameter decoder-only language model trained from scratch on approximately 20 billion tokens across four languages. Our contributions are:
...
\paragraph{Arabic Language Models.} Jais \citep{sengupta2023jais} is a 13B parameter bilingual Arabic-English model trained from scratch on 395 billion tokens, representing the most significant dedicated Arabic LLM effort. AraGPT2 \citep{antoun2021aragpt2} provided GPT-2 scale Arabic models. ArabianGPT \citep{koubaa2024arabiangpt} focuses on native Arabic without English token contamination. None of these include Hebrew or Farsi.

\paragraph{Hebrew-Arabic Models.} HeArBERT \citep{rivas2024hearbert} explores bilingual Arabic-Hebrew modeling through transliteration-based script unification, mapping Hebrew text into Arabic script to create a unified representation space. However, it is limited to encoder-only (BERT) architecture and does not support generation. To our knowledge, no prior work has trained a generative decoder-only model jointly on Hebrew, Arabic, and Farsi from scratch.

\paragraph{Small Multilingual Models.} SmolLM3 \citep{allal2025smollm3} provides a detailed 3B parameter training recipe but targets European languages (English, French, Spanish, German, Italian, Portuguese). The ``Smol Training Playbook'' documents insights from training at this scale but does not address non-Latin script languages or Semitic-family-specific challenges.

\paragraph{Multilingual Adaptation.} Marco-LLM \citep{marco2024} extends Qwen2 to low-resource languages through continual pre-training rather than training from scratch. This approach inherits the base model's tokenizer, which may be suboptimal for the target languages.
...
\end{axis}
\end{tikzpicture}
\caption{Pretraining data distribution. Hebrew is intentionally overrepresented as the \emph{anchor language}---the hypothesis being that strong anchor representations transfer to linguistically related languages (Arabic), which our sentiment experiments confirm (Section~\ref{tab:sentiment}).}
\label{fig:data_dist}
\end{figure}
...
\midrule
BPB & Held-out test splits & 10K tokens & --- & Internal \\
...
Sentiment (HE) & Hebrew Sentiment & 200 & 2 & mteb/hebrew\_sentiment \\
Sentiment (AR) & Arabic Tweets & 200 & 2 & arbml/Sentiment\_Tweets \\
Sentiment (FA) & Persian Sentiment & 200 & 2 & sepidmnorozy/Persian \\
Sentiment (EN) & SST-2 subset & 200 & 2 & SST-2 \\
News & AG News subset & 200 & 4 & AG News \\
\bottomrule
\end{tabular}
\caption{Evaluation datasets. N/lang indicates samples per language used in evaluation. Sentiment and news evaluations use generative classification (model generates label text). Small evaluation sets (200 samples) are a limitation we acknowledge.}
\label{tab:datasets}
\end{table}

\paragraph{Evaluation methodology.} For classification tasks (sentiment, news), we use generative evaluation: the model receives an instruction prompt and generates a label. Outputs are matched against expected labels using case-insensitive substring matching. We report the fraction of correctly matched outputs over total samples. We acknowledge that this methodology may undercount correct classifications when the model produces semantically equivalent but textually different outputs, and that very low baseline scores (below random chance) likely reflect prompt/parsing issues rather than anti-correlated predictions. We discuss this further in Section~\ref{sec:limitations}. All evaluation scripts, exact prompts, and raw result files are provided in \texttt{eval/} in the repository.
...
\end{axis}
\end{tikzpicture}
\caption{Translation strategy tradeoff. G-trans (English-mediated parallel data) helps EN$\leftrightarrow$X but fails for direct X$\leftrightarrow$Y. G2-direct (direct parallel data) excels at trained pairs (AR$\leftrightarrow$FA) but collapses EN$\leftrightarrow$X capability. Practitioners must choose: invest in direct parallel data for the pairs they care about.}
...
\end{tikzpicture}
\caption{Cross-lingual retrieval accuracy heatmap (base model, 10-way retrieval, chance=10\%). Darker blue = higher accuracy. EN$\leftrightarrow$HE shows strongest alignment (90\%), while Farsi shows weak connectivity to all other languages---consistent with its typological isolation from the Semitic family.}
...
Hebrew only & 18.5 & \textbf{49.0} & 1.5 & 33.5 \\
All 4 languages & 18.0 & 43.0 & 1.5 & 18.5 \\
Arabic + Farsi only & \textbf{23.0} & \textbf{67.0} & 5.5 & 21.0 \\
\bottomrule
\end{tabular}
...
\end{axis}
\end{tikzpicture}
\caption{Cross-lingual sentiment transfer results. The Hebrew-only configuration (blue) improves Arabic from 5.5\% to 49\% (9$\times$) despite receiving \emph{zero} Arabic sentiment data. This demonstrates that the model learns language-agnostic sentiment representations that transfer across the Semitic family. Critically, Farsi (same script as Arabic, but Indo-European family) shows no transfer---confirming that \textbf{linguistic family, not script, drives transfer}.}
...
\begin{itemize}
    ...
    \item SemiticGPT (3B): \textasciitilde20B tokens, spot instances only
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Use the autoresearch framework} \citep{karpathy2026autoresearch} or similar. Cost: \textasciitilde\$10--50 for 50 proxy experiments.
...
\begin{itemize}
    \item \textbf{Architecture}: Standard decoder-only transformer with RoPE, SwiGLU, RMSNorm. Scaling: 3B parameters is achievable on a single 48GB GPU with gradient accumulation; 7B+ requires multi-GPU (FSDP).
...
\begin{enumerate}
    ...
    \item \textbf{Cross-lingual retrieval} (embedding similarity) --- measures implicit alignment between languages. No labeled data needed.
...
\subsection{Recipe Summary}

Table~\ref{tab:recipe} summarizes the key decisions and their recommended settings.
...
Optimizer & AdamW ($\beta_2$=0.95); validate Muon at scale before trusting proxy results \\
...
Translation data & Direct parallel only; English-mediated does not transfer \\
...
\paragraph{Future work.} (1) Scaling to 7B parameters with improved Arabic SFT data quality; (2) adding more Semitic languages (Amharic, Tigrinya); (3) exploring adapter-based extension to new languages post-pretraining; (4) investigating whether the Hebrew$\rightarrow$Arabic transfer effect persists at larger scales.

% ============================================================
\section{Reproducibility}

All code, configurations, and result artifacts are available at \url{https://github.com/fatherRonnen/semitic-gpt}. The repository is organized to mirror the paper's pipeline, enabling end-to-end tracing from claims to artifacts:
...
\midrule
Tokenizer (\S3.1) & \texttt{tokenizer/} & Config, training script, fertility report \\
Data (\S3.2) & \texttt{data/} & Collection scripts, source manifests \\
Proxy (\S2, Related) & \texttt{proxy/} & 32 experiment configs, results, rationale \\
Pretraining (\S3.4) & \texttt{pretrain/} & Training scripts, hyperparams, logs \\
SFT (\S3.5) & \texttt{sft/} & 6 config files, training logs, data recipes \\
Evaluation (\S4) & \texttt{eval/} & All scripts, prompts, result JSONs \\
...
The tag \texttt{v0.1-paper-draft} in the repository freezes the exact state of all configs and results corresponding to this paper.
